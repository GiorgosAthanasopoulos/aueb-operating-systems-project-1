...
// constants provided in `Input and data` section of the project requirements
// pdf
...
uint orderCount = 0;
uint failedOrders = 0;
// total service time of all orders(service time: time from the moment the order
// is registered until it is delivered)
uint totalServiceTime = 0;
// max serive time of a single order
uint maxServiceTime = 0;
// total cooling time(cooling time: tiem from the moment the pizzas are out of
// the oven and util they are delivered)
...
uint availableOvens;
// uint array that signifies whether the ith packer has finished packing the
// order simpler than pthread_cond
uint *packerDone;
...
// not using seed because rand_r generates the same number every function
// call(srand and rand dont)
uint rndRange(uint low, uint high) { return low + (rand() % (high - low + 1)); }
...
// number(when we create the order threads) and we cannot use the i index of the
// for loop for lifetime purposes, we just allocate memory and put the number in
// there
//
// short lifetime explanation: after we break out of the for-loop where we
// create the threads the memory that was given to the variable i will be
// deleted as it is out of scope/not used anymore if the thread then tries to
// access i, we ll get a segmentation fault
...
void *packers(void *arg) {
  ...
  sem_post(&packersSem);
  // NOTE: we dont need to wait(pthread_join) for the delivery men to deliver
  // the order in order to free our packers(sem_post). we do it afterwards
  // because we need to ensure that the thread exits successfilly.
